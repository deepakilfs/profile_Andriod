...
function init_add_activity() {
    ...
    $.ajax({
         ...
         url: WEB_BASE_URL + WEB_OPTS_METHOD_FETCH_PROJECTS,
...
//----------------
    ...
    $("#but_add_task_block").bind("click", function () {
        ...
        var block_html = '<div class="task_block" style="border-top: 1px solid #ccc;display: inline-block;    padding-top: 20px;">'
                ...
                + '<input type="text" name="projectId" id="input_project" class="form-control default_input input_project" style="text-transform:initial;">'
...
//set default value for time, if not set
  ...
  //      console.log('from data: ' + form_data);
        $.ajax({
            url: WEB_BASE_URL + WEB_OPTS_METHOD_ADD_ACTIVITY,
...
//                });
			...
			else if (data === 'invalid_time_spent') {
...
function showTimeSheetHistory(start) {
	...
	var ws_url = WEB_BASE_URL + WEB_OPTS_METHOD_TIMESHEET_HISTORY + "?start=" + start;
        ...
        $.each(json, function (key, obj1) {
           ...
           // $.each(obj, function (key1, obj1) {
                ...
                tmpHtml += '<div class="task_box">'
                        ...
                        + '<br/><br/><div class="sml_text">@ ' + obj1.activityDate + '</div></div>'
                        + '<div class=" clearfix"></div> '
                        + '</div>';
                counter++;
            });
